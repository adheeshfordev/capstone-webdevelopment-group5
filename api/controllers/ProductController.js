	...
	try {
		...
		const products = productsRaw.map((product) => {
			product.imageUrl = product.imageUrl
				? convertGsToHttps(product.imageUrl)
				: convertGsToHttps(
						"gs://pixelparadisecapstone.appspot.com/lander-denys-J72jCU2HuAM-unsplash.jpg",
					);
...
const updateProduct = [
	...
	async (req, res) => {
		try {
			const { releaseDate, ...otherFields } = req.body;
			const updateFields = {
				...otherFields,
